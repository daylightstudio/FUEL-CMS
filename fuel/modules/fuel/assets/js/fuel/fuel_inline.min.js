...
iframe.contentDocument;var docHeight=fuel.calcHeight(contentDoc);if($("#fuel_main_content_inner .form, #fuel_actions",contentDoc).length){var width1=$("#fuel_main_content_inner .form",contentDoc).outerWidth(false)+74;var width2=$("#fuel_actions",contentDoc).outerWidth(false);var docWidth=width1>width2?width1:width2;if(docWidth<MIN_WIDTH&&$("#fuel_actions",contentDoc).length)docWidth=MIN_WIDTH}else if($("#login",contentDoc).length)docWidth=$("#login",contentDoc).width();else docWidth=$(contentDoc).width();
if(docHeight==0)docHeight=$(contentDoc).height();$(iframe).height(docHeight);$(iframe).width(docWidth)}}function initMarkers(){$(".__fuel_edit__").remove();var markers=$(".__fuel_marker__");var toggleEditOff=true;if(markers.length>0){$body=$("body");markers.each(function(i){var $this=$(this);var module=$this.attr("data-module");if(module=="pagevariables"&&pageId!=0||module!="pagevariables"){$this.attr("id","__fuel_marker__"+i);var coords=getMarkerPosition($this);var varName=$this.attr("title");var newClass=
...
function moveMarkers(){var markers=$(".__fuel_marker__");markers.each(function(i){var $this=$(this);var coords=getMarkerPosition($this);$("#__fuel_edit__"+i).css({left:coords.x,top:coords.y});if($this.filter(":hidden").length!=0)$("#__fuel_edit__"+i).hide();else $("#__fuel_edit__"+i).show()});var editors=$(".__fuel_edit__:visible");editors.each(function(i){adjustPosition(editors,$(this),0)})}function getMarkerPosition(marker){var offset=marker.offset();var xCoord=offset.left;var yCoord=offset.top+
...
parseInt($compareObj.css("left")))<X_OFFSET){$compareObj.css("top",topPos+Y_OFFSET+"px");counter++;adjustPosition(editors,$obj,counter);return false}})}function initEditors(){var formAction="";var editors=$(".__fuel_edit__");var resetCss={height:"auto",width:"auto",opacity:1,display:"block"};var closeEditor=function(){if(activeEditor){var iframe=activeEditor.find("iframe")[0];var contentDoc=iframe.contentDocument;if($(".success",contentDoc).length)top.window.location.reload();else{activeEditor.removeClass("__fuel_edit_active__");
