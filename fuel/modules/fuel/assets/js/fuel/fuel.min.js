...
$(this).addClass("field_depth_1")})});$("label,.field_depth_"+depth,elem).each(function(j){var newName=$(this).attr("name");if(newName&&newName.length){newName=newName.replace(/([-_a-zA-Z0-9\[\]]+)\[\d+\]([-_a-zA-Z0-9\[\]]+)$/,"$1["+i+"]$2");if($(this).attr("type")=="file")newName=newName.replace(/([-_a-zA-Z0-9]+)_\d+_([-_a-zA-Z0-9]+)$/,"$1_"+i+"_$2");newName=newName.replace("[","[");newName=newName.replace("]","]");$(this).attr("name",newName);if($checked)setTimeout(function(){$checked.each(function(){$(this).attr("checked",
...
null)){newName=newName.replace(/([-_a-zA-Z0-9]+\[)\d+(\]\[[-_a-zA-Z0-9]+\]\[[-_a-zA-Z0-9]+\])/g,"$1"+parentIndex+"$2");if($(this).attr("type")=="file")newName=newName.replace(/([-_a-zA-Z0-9]+)_\d+_([-_a-zA-Z0-9]+_[-_a-zA-Z0-9]+)/g,"$1_"+parentIndex+"_$2");newName=newName.replace("[","[");newName=newName.replace("]","]");$(this).attr("name",newName)}var newId=$(this).attr("id");if(newId&&(newId.length&&parentIndex!=null)){newId=newId.replace(/([-_a-zA-Z]+)_\d+_([-_a-zA-Z]+_[-_a-zA-Z0-9]+_[-_a-zA-Z])/g,
"$1_"+parentIndex+"_$2");$(this).attr("id",newId)}})})};var createRemoveButton=function(elem){$elem=$(elem);if(!$elem.find("."+options.removeButtonClass).length){var $remove=$elem.find(options.removeSelector+":first");if($remove.length&&$remove.find(options.removeButtonClass).length==0)$remove.empty().append('<a href="#" class="'+options.removeButtonClass+'">'+options.removeButtonText+" </a>");else{$remove.remove(options.removeSelector);$elem.append('<a href="#" class="'+options.removeButtonClass+
'">'+options.removeButtonText+" </a>")}}$(document).on("click",options.repeatableSelector+" ."+options.removeButtonClass,function(e){var $this=$(this).closest(options.repeatableParentSelector);var max=$this.attr("data-max")?parseInt($this.attr("data-max")):null;var min=$this.attr("data-min")?parseInt($this.attr("data-min")):null;if(options.warnBeforeDelete==false||confirm(options.warnBeforeDeleteMessage)){$(this).closest(options.repeatableSelector).remove();var $children=$this.children(options.repeatableSelector);
