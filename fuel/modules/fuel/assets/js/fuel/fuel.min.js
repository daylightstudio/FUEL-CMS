...
settings.selectedEmptyString+"</div>";combo+='<ul id="'+rightID+'" class="supercomboselect_list">';combo+="</ul>";combo+="</div>";combo+='<div style="clear: both; height: 0px; line-height: 0px; font-size: 0px;"></div>';combo+="</div>";combo+="</div>";$(this).hide().after(combo);$("#"+leftID+" li").die();$("#"+rightID+" li").die();$(document).on("dblclick","#"+leftID+' li[class!="option_disabled"]',function(e){addSelectedToRight()});$(document).on("dblclick","#"+rightID+" li",function(e){removeSelectedFromRight()});
$(document).on("click","#"+leftID+" li",function(e){if($(this).hasClass("optgrp")===false)if(isCtrlDown)$(this).removeClass(settings.selectedClass);else markForMove(this,leftID)});$(document).on("click","#"+rightID+" li",function(e){if(isCtrlDown)$(this).removeClass(settings.selectedClass);else markForMove(this,rightID)});$(document).on("keydown",function(e){if(e.shiftKey)isShiftDown=true;else if(e.metaKey||e.ctrlKey)isCtrlDown=true;else if(e.keyCode==38){if(lastSelected)markForMove($(lastSelected).prev())}else if(e.keyCode==
40)if(lastSelected)markForMove($(lastSelected).next())});$(document).on("keyup",function(e){isShiftDown=false;isCtrlDown=false});$("#"+settings.wrapperId+" .csadd").click(function(){addSelectedToRight()});$("#"+settings.wrapperId+" .csremove").click(function(){removeSelectedFromRight()});$("#"+settings.wrapperId+" .supercomboselect_left_empty_msg, #"+settings.wrapperId+" .supercomboselect_right_empty_msg").hide();if(settings.isSortable)$("#"+rightID).sortable();if(settings.isSortable)theForm.submit(function(){onFormSubmit();
...
$.fn.numeric.keypress).unbind("blur",$.fn.numeric.blur)};$.fn.getSelectionStart=function(o){try{if(o.createTextRange){var r=document.selection.createRange().duplicate();r.moveEnd("character",o.value.length);if(r.text=="")return o.value.length;return o.value.lastIndexOf(r.text)}else return o.selectionStart}catch(e){}};$.fn.setSelection=function(o,p){if(typeof p=="number")p=[p,p];if(p&&(p.constructor==Array&&p.length==2))if(o.createTextRange){var r=o.createTextRange();r.collapse(true);r.moveStart("character",
p[0]);r.moveEnd("character",p[1]);r.select()}else if(o.setSelectionRange){o.focus();o.setSelectionRange(p[0],p[1])}}})(jQuery);
(function($){jQuery.fn.repeatable=function(o){var options=$.extend({addButtonText:"Add Another",addButtonClass:"add_another",removeButtonClass:"remove",removeButtonText:"Remove",repeatableSelector:".repeatable",repeatableParentSelector:".repeatable_container",removeSelector:".remove",contentSelector:".repeatable_content",warnBeforeDelete:true,warnBeforeDeleteMessage:"Are you sure you want to delete this item?",addNewTitle:"New",sortableSelector:".grabber",sortable:true,initDisplay:false,dblClickBehavior:"toggle",
max:null,min:null,depth:1,allowCollapsingContent:true,removeable:true},o||{});var $checked=null;var parseTemplate=function(elem,i){var $childTemplates=$(elem).find(options.repeatableSelector);var depth=$(elem).parent().attr("data-depth");var titleField=$(elem).parent().attr("data-title_field");var title=$(elem).find('input[name$="['+titleField+']"]').val();if(!depth)depth=0;var depthSuffix=depth>0?"_"+depth:"";$(".num"+depthSuffix,elem).html(i+1);$(".index"+depthSuffix,elem).html(i);$(".title"+depthSuffix,
elem).html(title);$(elem).find("label,input,textarea,select").addClass("field_depth_0");var parseAttribute=function(elem,attr){var newId=$(elem).attr(attr);if(newId&&newId.length){newId=newId.replace(/\{index\}/g,i);newId=newId.replace(/([-_a-zA-Z0-9]+_)\d+(_[-_a-zA-Z0-9]+)$/,"$1"+i+"$2");$(elem).attr(attr,newId)}};var $childRepeatables=$(elem).find(options.repeatableSelector);if(depth==0&&$childTemplates.length)$childRepeatables.each(function(i){$(this).find("input,textarea,select").each(function(j){$(this).removeClass("field_depth_0");
$(this).addClass("field_depth_1")})});$("label,.field_depth_"+depth,elem).each(function(j){var newName=$(this).attr("name");if(newName&&newName.length){newName=newName.replace(/([-_a-zA-Z0-9\[\]]+)\[\d+\]([-_a-zA-Z0-9\[\]]+)$/,"$1["+i+"]$2");newName=newName.replace(/([-_a-zA-Z0-9]+)_\d+_([-_a-zA-Z0-9]+)$/,"$1_"+i+"_$2");newName=newName.replace("[","[");newName=newName.replace("]","]");$(this).attr("name",newName);if($checked)setTimeout(function(){$checked.each(function(){$(this).attr("checked","checked")})},
0)}if($(this).is("label"))parseAttribute(this,"for");else{parseAttribute(this,"id");$(this).attr("data-index",i)}});var $parentElem=$(elem).has(options.repeatableSelector+" "+options.repeatableSelector);var parentIndex=null;if($parentElem.length>0)parentIndex=$parentElem.attr("data-index");if(depth==0&&$childTemplates.length)$childRepeatables.each(function(i){$(this).find("input,textarea,select").each(function(j){var newName=$(this).attr("name");if(newName&&(newName.length&&parentIndex!=null)){newName=
newName.replace(/([-_a-zA-Z0-9]+\[)\d+(\]\[[-_a-zA-Z0-9]+\]\[[-_a-zA-Z0-9]+\])/g,"$1"+parentIndex+"$2");newName=newName.replace(/([-_a-zA-Z0-9]+)_\d+_([-_a-zA-Z0-9]+_[-_a-zA-Z0-9]+)/g,"$1_"+parentIndex+"_$2");newName=newName.replace("[","[");newName=newName.replace("]","]");$(this).attr("name",newName)}var newId=$(this).attr("id");if(newId&&(newId.length&&parentIndex!=null)){newId=newId.replace(/([-_a-zA-Z]+)_\d+_([-_a-zA-Z]+_[-_a-zA-Z0-9]+_[-_a-zA-Z])/g,"$1_"+parentIndex+"_$2");$(this).attr("id",
newId)}})})};var createRemoveButton=function(elem){if(!options.removeable)return;$elem=$(elem);if(!$elem.children("."+options.removeButtonClass).length){$elem.append('<a href="#" class="'+options.removeButtonClass+'">'+options.removeButtonText+" </a>");var $remove=$elem.find(options.removeSelector+":first");if($remove.length&&$remove.find(options.removeButtonClass).length==0)$remove.empty().append('<a href="#" class="'+options.removeButtonClass+'">'+options.removeButtonText+" </a>");else{$remove.remove(options.removeSelector);
$elem.append('<a href="#" class="'+options.removeButtonClass+'">'+options.removeButtonText+" </a>")}}$(document).on("click",options.repeatableSelector+" ."+options.removeButtonClass,function(e){var $this=$(this).closest(options.repeatableParentSelector);var max=$this.attr("data-max")?parseInt($this.attr("data-max")):null;var min=$this.attr("data-min")?parseInt($this.attr("data-min")):null;if(options.warnBeforeDelete==false||confirm(options.warnBeforeDeleteMessage)){$(this).closest(options.repeatableSelector).remove();
...
$(this);var $repeatables=$this.children(options.repeatableSelector);if(!$this.is(".__applied__"))$repeatables.each(function(i){parseTemplate(this,i);createRemoveButton(this)});$parent=$this.parent();if(options.max)$this.attr("data-max",options.max);if(options.min)$this.attr("data-min",options.min);if(options.dblClickBehavior)$this.attr("data-dblclick",options.dblClickBehavior);if(options.initDisplay&&!$this.is(".__applied__")){$this.attr("data-init_display",options.init_display);if(options.initDisplay==
"first")$repeatables.find(options.contentSelector).not(":first").hide();else if(options.initDisplay=="none"||options.initDisplay=="closed")$repeatables.find(options.contentSelector).hide()}if($parent.find(options.addButtonClass).length==0&&!$this.hasClass("__applied__"))$parent.append('<a href="#" class="'+options.addButtonClass+'">'+options.addButtonText+" </a>");if(options.sortable)$this.sortable({cursor:"move",handle:options.sortableSelector,start:function(event,ui){$this.trigger({type:"sortStarted",
clonedNode:$this})},stop:function(event,ui){reOrder($(this));$this.trigger({type:"sortStopped",clonedNode:$this})}});if(!$this.hasClass("__applied__"))createAddButton($this);if($this.attr("data-max"))checkMax($this,options.max);if($this.attr("data-min"))checkMin($this,options.min);createCollapsingContent($this);index++;$this.addClass("__applied__");return this})}})(jQuery);
