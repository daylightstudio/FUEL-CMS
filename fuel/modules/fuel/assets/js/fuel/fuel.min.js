...
{str='[';var i,len=_obj.length;for(i=0;i<len-1;i++){str+=jQuery.serialize(_obj[i])+',';}
str+=serialize(_obj[i])+']';}
else
{str='{';var key;for(key in _obj){str+='"'+key+'":'+jQuery.serialize(_obj[key])+',';}
str=str.replace(/\,$/,'')+'}';}
...
return cookieValue;}};jQuery.supercookie=function(name,key,value,options){if(typeof value!='undefined'){var cookieObj=jQuery.supercookie(name);cookieObj[key]=value;var cookieVal=escape(jQuery.serialize(cookieObj));jQuery.cookie(name,cookieVal,options);}else{var cookie=jQuery.cookie(name);var cookieObj=null;if(cookie){eval('cookieObj = '+unescape(cookie));}
if(!cookieObj)cookieObj={};if(key){if(cookieObj[key]){return cookieObj[key];}else{return null;}}else{return cookieObj;}}};(function(jQuery){jQuery.hotkeys={version:"0.8",specialKeys:{8:"backspace",9:"tab",13:"return",16:"shift",17:"ctrl",18:"alt",19:"pause",20:"capslock",27:"esc",32:"space",33:"pageup",34:"pagedown",35:"end",36:"home",37:"left",38:"up",39:"right",40:"down",45:"insert",46:"del",96:"0",97:"1",98:"2",99:"3",100:"4",101:"5",102:"6",103:"7",104:"8",105:"9",106:"*",107:"+",109:"-",110:".",111:"/",112:"f1",113:"f2",114:"f3",115:"f4",116:"f5",117:"f6",118:"f7",119:"f8",120:"f9",121:"f10",122:"f11",123:"f12",144:"numlock",145:"scroll",191:"/",224:"meta"},shiftNums:{"`":"~","1":"!","2":"@","3":"#","4":"$","5":"%","6":"^","7":"&","8":"*","9":"(","0":")","-":"_","=":"+",";":": ","'":"\"",",":"<",".":">","/":"?","\\":"|"}};function keyHandler(handleObj){if(typeof handleObj.data!=="string"){return;}
...
jQuery.each(["keydown","keyup","keypress"],function(){jQuery.event.special[this]={add:keyHandler};});})(jQuery);jQuery.cookie=function(name,value,options){if(typeof value!='undefined'){options=options||{};if(value===null){value='';options.expires=-1;}
var expires='';if(options.expires&&(typeof options.expires=='number'||options.expires.toUTCString)){var date;if(typeof options.expires=='number'){date=new Date();date.setTime(date.getTime()+(options.expires*24*60*60*1000));}else{date=options.expires;}
expires='; expires='+date.toUTCString();}
var path=options.path?'; path='+(options.path):'';var domain=options.domain?'; domain='+(options.domain):'';var secure=options.secure?'; secure':'';document.cookie=[name,'=',encodeURIComponent(value),expires,path,domain,secure].join('');}else{var cookieValue=null;if(document.cookie&&document.cookie!=''){var cookies=document.cookie.split(';');for(var i=0;i<cookies.length;i++){var cookie=jQuery.trim(cookies[i]);if(cookie.substring(0,name.length+1)==(name+'=')){cookieValue=decodeURIComponent(cookie.substring(name.length+1));break;}}}
return cookieValue;}};;(function($){jQuery.fn.simpleTab=function(settings){var s=jQuery.extend({startIndex:0,childrenSelector:null,cookie:false},settings);var activeTab=null;var activeContent=null;var hasCookie=(s.cookie&&s.cookie.group&&s.cookie.name);return this.each(function(){var _this=this;$(this).find('a').each(function(i){var id=$(this).attr('href');$(id).hide();})
$children=(s.childrenSelector)?$(this).find(s.childrenSelector):$(this).children();$children.click(function(){$this=$(this);var index=$this.index();if(isNaN(index))index=0;if(!activeTab||$this.find('a').attr('href')!=activeTab.find('a').attr('href')){if(activeTab)activeTab.removeClass('active');if(activeContent)activeContent.hide();$this.addClass('active');var id=$(this).find('a').attr('href');activeContent=$(id);activeContent.show();activeTab=$this;$(_this).trigger('tabClicked',[index,activeTab,activeContent,s]);if(hasCookie){$.supercookie(s.cookie.group,s.cookie.name,index.toString(),s.cookie.params);}else{s.startIndex=0;}}
return false;});if(hasCookie){s.startIndex=parseInt($.supercookie(s.cookie.group,s.cookie.name));}
if(isNaN(s.startIndex)||($children.size()-1)<s.startIndex)s.startIndex=0;$(this).children().eq(s.startIndex).click();return this;});};})(jQuery);jQuery.tableDnD={currentTable:null,dragObject:null,mouseOffset:null,oldY:0,build:function(options){this.each(function(){this.tableDnDConfig=jQuery.extend({onDragStyle:null,onDropStyle:null,onDragClass:"tDnD_whileDrag",onDrop:null,onDragStart:null,scrollAmount:5,serializeRegexp:/[^\-]*$/,serializeParamName:null,dragHandle:null},options||{});jQuery.tableDnD.makeDraggable(this);});jQuery(document).bind('mousemove',jQuery.tableDnD.mousemove).bind('mouseup',jQuery.tableDnD.mouseup);return this;},makeDraggable:function(table){var config=table.tableDnDConfig;if(table.tableDnDConfig.dragHandle){var cells=jQuery("td."+table.tableDnDConfig.dragHandle,table);cells.each(function(){jQuery(this).mousedown(function(ev){jQuery.tableDnD.dragObject=this.parentNode;jQuery.tableDnD.currentTable=table;jQuery.tableDnD.mouseOffset=jQuery.tableDnD.getMouseOffset(this,ev);if(config.onDragStart){config.onDragStart(table,this);}
...
var currentRow=jQuery.tableDnD.findDropTargetRow(dragObj,y);if(currentRow){if(movingDown&&jQuery.tableDnD.dragObject!=currentRow){try{jQuery.tableDnD.dragObject.parentNode.insertBefore(jQuery.tableDnD.dragObject,currentRow.nextSibling);}catch(e)
{}}else if(!movingDown&&jQuery.tableDnD.dragObject!=currentRow){try{jQuery.tableDnD.dragObject.parentNode.insertBefore(jQuery.tableDnD.dragObject,currentRow);}catch(e)
{}}}}
...
jQuery.fn.extend({tableDnD:jQuery.tableDnD.build,tableDnDUpdate:jQuery.tableDnD.updateTables,tableDnDSerialize:jQuery.tableDnD.serializeTables});;(function(f,h,$){var a='placeholder'in h.createElement('input'),d='placeholder'in h.createElement('textarea'),i=$.fn,c=$.valHooks,k,j;if(a&&d){j=i.placeholder=function(){return this};j.input=j.textarea=true}else{j=i.placeholder=function(){return this.filter((a?'textarea':':input')+'[placeholder]').not('.placeholder').bind({'focus.placeholder':b,'blur.placeholder':e}).data('placeholder-enabled',true).trigger('blur.placeholder').end()};j.input=a;j.textarea=d;k={get:function(m){var l=$(m);return l.data('placeholder-enabled')&&l.hasClass('placeholder')?'':m.value},set:function(m,n){var l=$(m);if(!l.data('placeholder-enabled')){return m.value=n}if(n==''){m.value=n;e.call(m)}else{if(l.hasClass('placeholder')){b.call(m,true,n)||(m.value=n)}else{m.value=n}}return l}};a||(c.input=k);d||(c.textarea=k);$(function(){$(h).delegate('form','submit.placeholder',function(){var l=$('.placeholder',this).each(b);setTimeout(function(){l.each(e)},10)})});$(f).bind('beforeunload.placeholder',function(){$('.placeholder').each(function(){this.value=''})})}function g(m){var l={},n=/^jQuery\d+$/;$.each(m.attributes,function(p,o){if(o.specified&&!n.test(o.name)){l[o.name]=o.value}});return l}function b(m,n){var l=this,o=$(l);if(l.value==o.attr('placeholder')&&o.hasClass('placeholder')){if(o.data('placeholder-password')){o=o.hide().next().show().attr('id',o.removeAttr('id').data('placeholder-id'));if(m===true){return o[0].value=n}o.focus()}else{l.value='';o.removeClass('placeholder')}}}function e(){var q,l=this,p=$(l),m=p,o=this.id;if(l.value==''){if(l.type=='password'){if(!p.data('placeholder-textinput')){try{q=p.clone().attr({type:'text'})}catch(n){q=$('<input>').attr($.extend(g(this),{type:'text'}))}q.removeAttr('name').data({'placeholder-password':true,'placeholder-id':o}).bind('focus.placeholder',b);p.data({'placeholder-textinput':q,'placeholder-id':o}).before(q)}p=p.removeAttr('id').hide().prev().attr('id',o).show()}p.addClass('placeholder');p[0].value=p.attr('placeholder')}else{p.removeClass('placeholder')}}}(this,document,jQuery));(function($){$.extend({selso:{defaults:{type:'alpha',orderBy:'span.value',direction:'asc'},extractVal:function(type,text){if(type=='num'){return 1*text;}
return text;},accentsTidy:function(s){var r=s.toLowerCase();r=r.replace(new RegExp(/\s/g),"");r=r.replace(new RegExp(/[Ãƒ ÃƒÂ¡ÃƒÂ¢ÃƒÂ£ÃƒÂ¤ÃƒÂ¥]/g),"a");r=r.replace(new RegExp(/ÃƒÂ¦/g),"ae");r=r.replace(new RegExp(/ÃƒÂ§/g),"c");r=r.replace(new RegExp(/[ÃƒÂ¨ÃƒÂ©ÃƒÂªÃƒÂ«]/g),"e");r=r.replace(new RegExp(/[ÃƒÂ¬ÃƒÂ­ÃƒÂ®ÃƒÂ¯]/g),"i");r=r.replace(new RegExp(/ÃƒÂ±/g),"n");r=r.replace(new RegExp(/[ÃƒÂ²ÃƒÂ³ÃƒÂ´ÃƒÂµÃƒÂ¶]/g),"o");r=r.replace(new RegExp(/Ã…â€œ/g),"oe");r=r.replace(new RegExp(/[ÃƒÂ¹ÃƒÂºÃƒÂ»ÃƒÂ¼]/g),"u");r=r.replace(new RegExp(/[ÃƒÂ½ÃƒÂ¿]/g),"y");r=r.replace(new RegExp(/\W/g),"");return r;},orderAlpha:function(a,b){var l=Math.min(a.length,b.length);for(var i=0;i<l;i++){if(a.charAt(i)<b.charAt(i))return-1;else if(a.charAt(i)>b.charAt(i))return 1;}
if(a.length>b.length)return 1;if(a.length<b.length)return-1;return 0;},alphaGreaterThan:function(s1,s2){return this.orderAlpha(s1,s2);},stablesort:function(a,of){var r=a;var s;for(var i=1;i<r.length;i++){var j=1;while(i>=j&&of(r[i-j],r[i])>0){j++;}
if(j>0){s=r.slice(0,i-j+1);s.push(r[i]);s=s.concat(r.slice(i-j+1,i));if(i<r.length-1){s=s.concat(r.slice(i+1));}
r=s;}}
return r;}}});$.fn.extend({outhtml:function(){if(this.length)
return $('<div>').append($(this[0]).clone()).html();return null;},prependToParent:function(){return this.each(function(){$(this).parent().prepend($(this).remove());});},selso:function(settings){var tt=settings.type||$.selso.defaults.type;var to=settings.orderBy||$.selso.defaults.orderBy;var td=settings.direction||$.selso.defaults.direction;var te=settings.extract;var of=settings.orderFn;if(!$.isFunction(te)){te=function(obj){return $.selso.extractVal(tt,$(to,obj).text());};}
var arr=[];this.each(function(){arr.unshift({id:this.id,val:te(this)});});if($.isFunction(of)){}
else if(tt=='num'){of=function(a,b){return a.val-b.val;};}
else{if(tt=='accents'){$.map(arr,function(n){n.val=$.selso.accentsTidy(n.val);});}
of=function(a,b){return $.selso.alphaGreaterThan(a.val,b.val);};}
var off=of;if(td=='asc'){off=function(a,b){return-1*of(a,b);}}
arr=$.selso.stablesort(arr,off);for(var i=0;i<arr.length;i++){$('#'+arr[i].id).prependToParent();}
return this;}});})(jQuery);(function($){if($.browser.mozilla){$.fn.disableTextSelect=function(){return this.each(function(){$(this).css({"MozUserSelect":"none"})})};$.fn.enableTextSelect=function(){return this.each(function(){$(this).css({"MozUserSelect":""})})}}else{if($.browser.msie){$.fn.disableTextSelect=function(){return this.each(function(){$(this).bind("selectstart.disableTextSelect",function(){return false})})};$.fn.enableTextSelect=function(){return this.each(function(){$(this).unbind("selectstart.disableTextSelect")})}}else{$.fn.disableTextSelect=function(){return this.each(function(){$(this).bind("mousedown.disableTextSelect",function(){return false})})};$.fn.enableTextSelect=function(){return this.each(function(){$(this).unbind("mousedown.disableTextSelect")})}}}})(jQuery);(function($){jQuery.fn.supercomboselect=function(settings){settings=jQuery.extend({addButton:' &rarr; ',removeButton:' &larr; ',optionsIdPrefix:'auto',wrapperId:'auto',selectedClass:'selected',isSortable:false,isSearchable:'auto',searchableAutoLimit:10,autoSort:true,valuesEmptyString:'There are no more values to select',selectedEmptyString:'Select from the values on the left',defaultSearchBoxString:'Filter your search',minNumOfSearchChars:3,selectedOrdering:[]},settings);this.each(function(i){var _this=this;var selectID=this.id;var leftID=selectID+'_left';var rightID=selectID+'_right';var isShiftDown=false;var isCtrlDown=false;var lastSelected=null;var theForm=$(this).parents('form');var combo='';var refs;var searchBox=null;if(!settings.optionsIdPrefix||settings.optionsIdPrefix=='auto'||settings.optionsIdPrefix.length===0){settings.optionsIdPrefix=selectID+'_';}
if(!settings.wrapperId||settings.wrapperId=='auto'||settings.optionsIdPrefix.length===0){settings.wrapperId=selectID+'_wrapper';}
if(settings.isSearchable=='auto'){if($('#'+selectID).find('option').length>=settings.searchableAutoLimit){settings.isSearchable=true;}else{settings.isSearchable=false;}}
var customSelectedSorting=(settings.isSortable&&settings.selectedOrdering);$('#'+settings.wrapperId).remove();combo+='<div id="'+settings.wrapperId+'" class="supercomboselect_wrapper">';combo+='<div class="supercomboselect_left">';combo+='<div class="supercomboselect">';combo+='<div class="supercomboselect_empty_msg supercomboselect_left_empty_msg">'+settings.valuesEmptyString+'</div>';combo+='<ul id="'+leftID+'" class="supercomboselect_list">';combo+='</ul>';combo+='</div>';combo+='</div>';combo+='<div class="supercomboselect_btns">';combo+='<input type="button" class="csadd" value="'+settings.addButton+'" />';combo+='<input type="button" class="csremove" value="'+settings.removeButton+'" />';combo+='</div>';combo+='<div class="supercomboselect_right">';combo+='<div class="supercomboselect">';combo+='<div class="supercomboselect_empty_msg supercomboselect_right_empty_msg">'+settings.selectedEmptyString+'</div>';combo+='<ul id="'+rightID+'" class="supercomboselect_list">';combo+='</ul>';combo+='</div>';combo+='<div style="clear: both; height: 0px; line-height: 0px; font-size: 0px;"></div>';combo+='</div>';combo+='</div>';$(this).hide().after(combo);$('#'+leftID+' li').die();$('#'+rightID+' li').die();$('#'+leftID+' li[class!="option_disabled"]').live('dblclick',function(e){addSelectedToRight();});$(document).on('dblclick','#'+rightID+' li',function(e){removeSelectedFromRight();});$('#'+leftID+' li').live('click',function(e){if($(this).hasClass('optgrp')===false)
{if(isCtrlDown){$(this).removeClass(settings.selectedClass);}else{markForMove(this,leftID);}}});$(document).on('click','#'+rightID+' li',function(e){if(isCtrlDown){$(this).removeClass(settings.selectedClass);}else{markForMove(this,rightID);}});$(document).keydown(function(e){if(e.shiftKey){isShiftDown=true;}else if(e.metaKey||e.CtrlKey){isCtrlDown=true;}else if(e.keyCode==38){if(lastSelected)markForMove($(lastSelected).prev());}else if(e.keyCode==40){if(lastSelected)markForMove($(lastSelected).next());}});$(document).keyup(function(e){isShiftDown=false;isCtrlDown=false;});$('#'+settings.wrapperId+' .csadd').click(function(){addSelectedToRight();});$('#'+settings.wrapperId+' .csremove').click(function(){removeSelectedFromRight();});$('#'+settings.wrapperId+' .supercomboselect_left_empty_msg, #'+settings.wrapperId+' .supercomboselect_right_empty_msg').hide();if(settings.isSortable){$('#'+rightID).sortable();}
if(settings.isSortable){theForm.submit(function(){onFormSubmit();return true;});}
theForm.bind('form-pre-serialize',function(){onFormSubmit();});if(settings.isSearchable){var prevSearchText='';var nonSearchableChars=[13,37,38,39,40,18,17];var searchBoxHTML='<div class="supercomboselect_search">';searchBoxHTML+='<input type="text" value="" name="supercomboselect_search" class="supercomboselect_search_text" /> <a href="#" class="supercomboselect_search_clear" style="display: none;">Clear Filter</a>';searchBoxHTML+='</div>';$('#'+leftID).parent().after(searchBoxHTML);searchBox=$('#'+leftID).parent().parent().find('.supercomboselect_search_text');searchBox.attr('placeholder',settings.defaultSearchBoxString).keyup(function(e){isShiftDown=false;if(e.shiftKey||e.metaKey||e.altKey||e.ctrlKey||$.inArray(e.keyCode,nonSearchableChars)!=-1)return false;var searchTerm=searchBox.val(),$searchClear=searchBox.siblings('.supercomboselect_search_clear');$searchClear.hide();if(searchTerm.length){$searchClear.show();}
if(searchTerm.length>=settings.minNumOfSearchChars||searchTerm.length===0){if(searchTerm.substr(0,(prevSearchText.length))!=prevSearchText||searchTerm.length===0){refreshLists();}
var val=$(this).val().toLowerCase();if(val.length){var filtered=$('#'+leftID+' li:not(.optgrp)').filter(function(){var text=$(this).data('label');if(!text){return false;}
var index=text.toLowerCase().indexOf(val);if(index==-1){return false;}else{var highlightedText='';highlightedText+=text.substr(0,index);highlightedText+='<span class="supercomboselect_search_highlight">'+text.substr(index,val.length)+'</span>';highlightedText+=text.substr((index+val.length));$(this).html(highlightedText);return true;}});$('#'+leftID).empty().append(filtered);}}else if(searchTerm.length==(settings.minNumOfSearchChars-1)&&searchTerm.substr(0,(prevSearchText.length))!=prevSearchText&&searchTerm.length!==0){refreshLists();}
prevSearchText=searchTerm;return false;}).keydown(function(e){if(e.keyCode==13){$(e.currentTarget).blur();return false;}});searchBox.siblings('.supercomboselect_search_clear').click(function(e){e.preventDefault();refreshLists();searchBox.val('');$(this).hide();});}
function refreshLists(init){var leftOpts=[];var rightOpts=[];refs=[];var idNum=0;var maxSelected=(typeof settings.selectedOrdering=='boolean')?0:settings.selectedOrdering.length;var flippedSelectedOrder={};if(customSelectedSorting&&typeof settings.selectedOrdering=='object'){for(var n in settings.selectedOrdering){flippedSelectedOrder[settings.selectedOrdering[n]]=n;}}
var $select=$('#'+selectID),$optgroup=$select.find('optgroup');$select.find('option').each(function(i){var text=$(this).text(),value=$(this).attr('value'),id=(settings.optionsIdPrefix+idNum),opt='',$opt=$('<li/>');$opt.text(text);$opt.attr('id',id);$opt.attr('data-label',text);if($(this).is(':disabled')){$opt.addClass('option_disabled');}
if(customSelectedSorting)
{var optSpanVal=(flippedSelectedOrder[value])?flippedSelectedOrder[value]:maxSelected;$opt.append('<span/>');$opt.find('span').attr('id',id+'_val').text(optSpanVal).hide();}
opt=$opt[0].outerHTML;if(!$(this).is('[selected]')&&$(this).parent().is('optgroup')&&($(this).parent().find('option:not([selected])')[0]===$(this)[0]))
{opt='<li class="optgrp">'+$(this).parent().attr('label')+'</li>'+opt;}
if($(this).attr('selected')){rightOpts.push(opt);}else{leftOpts.push(opt);}
refs[idNum]=this;idNum++;});leftOpts=leftOpts.join("\n");rightOpts=rightOpts.join("\n");theForm.find('#'+leftID).empty().append(leftOpts);if(!customSelectedSorting||init)theForm.find('#'+rightID).empty().append(rightOpts);$('#'+settings.wrapperId+' .supercomboselect_left_empty_msg').hide();$('#'+settings.wrapperId+' .supercomboselect_right_empty_msg').hide();if(!$('#'+leftID).children().length){$('#'+settings.wrapperId+' .supercomboselect_left_empty_msg').show();}
if(!$('#'+rightID).children().length){$('#'+settings.wrapperId+' .supercomboselect_right_empty_msg').show();}
isShiftDown=false;isCtrlDown=false;$('.supercomboselect').css({overflowY:'hidden'}).css({overflowY:'auto'}).disableTextSelect();autoSort();}
function addSelectedToRight(){$selectedOpts=$('#'+leftID+' li[class='+settings.selectedClass+']');var selected=[];$selectedOpts.each(function(i){if(customSelectedSorting)theForm.find('#'+rightID).append($(this).removeClass(settings.selectedClass).html($(this).html()));var opt=$(getOptionSourceRef(getIdNum(this)));opt.attr('selected','selected');selected.push(opt.attr('value'));});$(_this).trigger('selectionAdded',[selected]);if(searchBox&&searchBox.val().length){searchBox.val('');searchBox.focus();}
refreshLists();}
function removeSelectedFromRight(){$selectedOpts=$('#'+rightID+' li[class='+settings.selectedClass+']');var selected=[];$selectedOpts.each(function(i){if(customSelectedSorting)$(this).remove();var opt=$(getOptionSourceRef(getIdNum(this)));opt.removeAttr('selected','selected');selected.push(opt.attr('value'));});$(_this).trigger('selectionRemoved',[selected]);refreshLists();}
function markForMove(selector){if(isShiftDown&&lastSelected){var lis=$(selector).parents('ul').find('li');var num1=lis.index($(selector));var num2=lis.index($(lastSelected));if(num1<num2){startNum=num1;endNum=num2;}else{startNum=num2;endNum=num1;}
if(startNum>-1){lis.slice(startNum,(endNum+1)).addClass(settings.selectedClass);}}else{$(selector).addClass(settings.selectedClass);lastSelected=selector;}
if(searchBox){searchBox.blur();}}
function getOptionSourceRef(idNum){return refs[idNum];}
function getIdNum(selector){var id=$(selector).attr('id');var idNum=(id)?parseInt(id.substr(settings.optionsIdPrefix.length),10):0;return idNum;}
function autoSort(){if(settings.autoSort&&!settings.isSortable){var beginIndex=settings.optionsIdPrefix.length;$('#'+rightID).find('li').selso({type:'alpha',extract:function(o){return $(o).text().toLowerCase();}});}}
function onFormSubmit(){$('#'+rightID+' li').each(function(i){var src=$(getOptionSourceRef(getIdNum(this)));$(this).prepend('<input type="hidden" name="'+$('#'+selectID).attr('name')+'" value="'+src.attr('value')+'"" class="sorted_val" />');});$('#'+selectID).remove();}
refreshLists(true);if(customSelectedSorting){$('#'+rightID).find('li').selso({type:'num',orderBy:'span',direction:'asc'});}});return this;};})(jQuery);;if(window.jQuery)(function($){$.fn.MultiFile=function(options){if(this.length==0)return this;if(typeof arguments[0]=='string'){if(this.length>1){var args=arguments;return this.each(function(){$.fn.MultiFile.apply($(this),args);});};$.fn.MultiFile[arguments[0]].apply(this,$.makeArray(arguments).slice(1)||[]);return this;};var options=$.extend({},$.fn.MultiFile.options,options||{});$('form').not('MultiFile-intercepted').addClass('MultiFile-intercepted').submit($.fn.MultiFile.disableEmpty);if($.fn.MultiFile.options.autoIntercept){$.fn.MultiFile.intercept($.fn.MultiFile.options.autoIntercept);$.fn.MultiFile.options.autoIntercept=null;};this.not('.MultiFile-applied').addClass('MultiFile-applied').each(function(){window.MultiFile=(window.MultiFile||0)+1;var group_count=window.MultiFile;var MultiFile={e:this,E:$(this),clone:$(this).clone()};if(typeof options=='number')options={max:options};var o=$.extend({},$.fn.MultiFile.options,options||{},($.metadata?MultiFile.E.metadata():($.meta?MultiFile.E.data():null))||{},{});if(!(o.max>0)){o.max=MultiFile.E.attr('maxlength');if(!(o.max>0)){o.max=(String(MultiFile.e.className.match(/\b(max|limit)\-([0-9]+)\b/gi)||['']).match(/[0-9]+/gi)||[''])[0];if(!(o.max>0))o.max=-1;else o.max=String(o.max).match(/[0-9]+/gi)[0];}};o.max=new Number(o.max);o.accept=o.accept||MultiFile.E.attr('accept')||'';if(!o.accept){o.accept=(MultiFile.e.className.match(/\b(accept\-[\w\|]+)\b/gi))||'';o.accept=new String(o.accept).replace(/^(accept|ext)\-/i,'');};$.extend(MultiFile,o||{});MultiFile.STRING=$.extend({},$.fn.MultiFile.options.STRING,MultiFile.STRING);$.extend(MultiFile,{n:0,slaves:[],files:[],instanceKey:MultiFile.e.id||'MultiFile'+String(group_count),generateID:function(z){return MultiFile.instanceKey+(z>0?'_F'+String(z):'');},trigger:function(event,element){var handler=MultiFile[event],value=$(element).attr('value');if(handler){var returnValue=handler(element,value,MultiFile);if(returnValue!=null)return returnValue;}
...
{o.focus();o.setSelectionRange(p[0],p[1]);}}}})(jQuery);;(function($){jQuery.fn.repeatable=function(o){var options=$.extend({addButtonText:'Add Another',addButtonClass:'add_another',removeButtonClass:'remove',removeButtonText:'Remove',repeatableSelector:'.repeatable',contentSelector:'.repeatable_content',warnBeforeDelete:true,warnBeforeDeleteMessage:'Are you sure you want to delete this item?',addNewTitle:'New',sortableSelector:'.grabber',sortable:true,initDisplay:false,dblClickBehavior:'toggle',max:null,min:null,depth:1,allowCollapsingContent:true},o||{});var $checked=null;var parseTemplate=function(elem,i){var $childTemplates=$(elem).find(options.repeatableSelector);var depth=$(elem).parent().attr('data-depth');var titleField=$(elem).parent().attr('data-title_field');var title=$(elem).find('input[name$="['+titleField+']"]').val();if(!depth)depth=0;var depthSuffix=(depth>0)?'_'+depth:'';$('.num'+depthSuffix,elem).html((i+1));$('.index'+depthSuffix,elem).html(i);$('.title'+depthSuffix,elem).html(title);$(elem).find('label,input,textarea,select').addClass('field_depth_0');var parseAttribute=function(elem,attr){var newId=$(elem).attr(attr);if(newId&&newId.length){newId=newId.replace(/\{index\}/g,i);newId=newId.replace(/([-_a-zA-Z0-9]+_)\d+(_[-_a-zA-Z0-9]+)$/,'$1'+i+'$2');$(elem).attr(attr,newId);}}
var $childRepeatables=$(elem).find(options.repeatableSelector);if(depth==0&&$childTemplates.length){$childRepeatables.each(function(i){$(this).find('input,textarea,select').each(function(j){$(this).removeClass('field_depth_0');$(this).addClass('field_depth_1');});});}
$('label,.field_depth_'+depth,elem).each(function(j){var newName=$(this).attr('name');if(newName&&newName.length){newName=newName.replace(/([-_a-zA-Z0-9\[\]]+)\[\d+\]([-_a-zA-Z0-9\[\]]+)$/,'$1['+i+']$2');newName=newName.replace('[','\[');newName=newName.replace(']','\]');$(this).attr('name',newName);if($checked){setTimeout(function(){$checked.each(function(){$(this).attr('checked','checked');});},0);}}
if($(this).is('label')){parseAttribute(this,'for');}else{parseAttribute(this,'id');$(this).attr('data-index',i);}})
var $parentElem=$(elem).has(options.repeatableSelector+' '+options.repeatableSelector);var parentIndex=null;if($parentElem.length>0){parentIndex=$parentElem.attr('data-index');}
if(depth==0&&$childTemplates.length){$childRepeatables.each(function(i){$(this).find('input,textarea,select').each(function(j){var newName=$(this).attr('name')
if(newName&&newName.length&&parentIndex!=null){newName=newName.replace(/([-_a-zA-Z0-9]+\[)\d+(\]\[[-_a-zA-Z0-9]+\]\[[-_a-zA-Z0-9]+\])/g,'$1'+parentIndex+'$2');newName=newName.replace('[','\[');newName=newName.replace(']','\]');$(this).attr('name',newName);}
var newId=$(this).attr('id')
if(newId&&newId.length&&parentIndex!=null){newId=newId.replace(/([-_a-zA-Z]+)_\d+_([-_a-zA-Z]+_[-_a-zA-Z0-9]+_[-_a-zA-Z])/g,'$1_'+parentIndex+'_$2');$(this).attr('id',newId);}})})}}
var createRemoveButton=function(elem){if($(elem).children('.'+options.removeButtonClass).length==0){$(elem).append('<a href="#" class="'+options.removeButtonClass+'">'+options.removeButtonText+' </a>');}
$(document).on('click',options.repeatableSelector+' .'+options.removeButtonClass,function(e){var $this=$(this).closest(options.repeatableSelector).parent();var max=($this.attr('data-max'))?parseInt($this.attr('data-max')):null;var min=($this.attr('data-min'))?parseInt($this.attr('data-min')):null;if(options.warnBeforeDelete==false||confirm(options.warnBeforeDeleteMessage)){$(this).parent().remove();var $children=$this.children(options.repeatableSelector);if($children.length<max){$this.next().show();}
reOrder($this);}
checkMin($this,min);$this.trigger('removed');e.stopImmediatePropagation();return false;});}
var reOrder=function($elem){$checked=$('input[type="radio"]',$elem).filter(':checked');$elem.children(options.repeatableSelector).each(function(i){$(this).attr('data-index',i);parseTemplate(this,i);});}
var checkMax=function($elem,max){if(max&&$elem.children(options.repeatableSelector).length!=0&&$elem.children(options.repeatableSelector).length>=max){$elem.next().hide();}else{$elem.next().show();}}
var checkMin=function($elem,min){$children=$elem.children(options.repeatableSelector);min=parseInt(min);if(min&&$children.length!=0&&$children.length<=min){$children.find('.'+options.removeButtonClass+':first').hide();}else{$children.find('.'+options.removeButtonClass+':first').show();}}
var cloneRepeatableNode=function($elem){$clone=$elem.children(options.repeatableSelector+':first').clone(false);return $clone;}
var createCollapsingContent=function($elem){if(options.allowCollapsingContent){$($elem).find(options.sortableSelector).unbind('dblclick').dblclick(function(e){$parent=$(this).closest(options.repeatableSelector).parent();var dblclick=($parent.attr('data-dblclick'))?$parent.attr('data-dblclick'):null;if(dblclick=='accordion'||dblclick=='accordian'){$parent.find(options.contentSelector).hide();$(this).closest(options.repeatableSelector).find(options.contentSelector+':first').show();}else{$(this).closest(options.repeatableSelector).find(options.contentSelector+':first').toggle();}})}}
var createAddButton=function($elem){$('.'+options.addButtonClass).each(function(i){var $prev=$(this).prev();var $clone=cloneRepeatableNode($prev);$(this).data('clone',$clone);});}
$(document).on('click','.'+options.addButtonClass,function(e){e.preventDefault();e.stopImmediatePropagation();var $prev=$(this).prev();var max=($prev.attr('data-max'))?parseInt($prev.attr('data-max')):null;var min=($prev.attr('data-min'))?parseInt($prev.attr('data-min')):null;var dblclick=($prev.attr('data-dblclick'))?$prev.attr('data-dblclick'):null;if(!$(e.currentTarget).data('clone')){var $clone=cloneRepeatableNode($prev);$(e.currentTarget).data('clone',$clone);}else{var $clone=$(e.currentTarget).data('clone');}
var $this=$(this).prev();var $clonecopy=$clone.clone(false);$clonecopy.addClass('noclone');$clonecopy.find(options.contentSelector+':first').show();if(dblclick=='accordian'||dblclick=='accordion'){$prev.find(options.contentSelector).hide();}
createCollapsingContent($clonecopy);var $children=$this.children(options.repeatableSelector);if(max&&$children.length>=max){return false;}
var index=$children.length;parseTemplate($clonecopy,index);createRemoveButton($clonecopy);$this.append($clonecopy);$clonecopy.find('input,select,textarea').not('input[type="radio"], input[type="checkbox"], input[type="button"]').val('');$clonecopy.find('.title, .num, .index').html('');$clonecopy.find('.noclone').remove();reOrder($this);if(max&&$children.length!=0&&$children.length>=(max-1)){$(this).hide();}
checkMin($prev,min);$this.trigger({type:'cloned',clonedNode:$clonecopy});});var index=0;return this.each(function(){var $this=$(this);var $repeatables=$this.children(options.repeatableSelector);$repeatables.each(function(i){parseTemplate(this,i);createRemoveButton(this);});$parent=$this.parent();if(options.max){$this.attr('data-max',options.max);}
...
if(options.initDisplay){$this.attr('data-init_display',options.init_display);if(options.initDisplay=='first'){$repeatables.find('.repeatable_content').not(':first').hide();}else if(options.initDisplay=='none'||options.initDisplay=='closed'){$repeatables.find('.repeatable_content').hide();}}
...
if(options.sortable){$this.sortable({cursor:'move',handle:options.sortableSelector,start:function(event,ui){$this.trigger({type:'sortStarted',clonedNode:$this});},stop:function(event,ui){reOrder($(this));$this.trigger({type:'sortStopped',clonedNode:$this});}});}
if(!$this.hasClass('__applied__')){createAddButton($this);}
if($this.attr('data-max'))checkMax($this,options.max);if($this.attr('data-min'))checkMin($this,options.min);createCollapsingContent($this);index++;$this.addClass('__applied__');return this;});};})(jQuery);(function($){var pushStackOrig,pushStackChrome;pushStackOrig=$.fn.pushStack;pushStackChrome=function(elems,name,selector){var ret=new jQuery.fn.init();if(jQuery.isArray(elems)){push.apply(ret,elems);}else{jQuery.merge(ret,elems);}
ret.prevObject=this;ret.context=this.context;if(name==="find"){ret.selector=this.selector+(this.selector?" ":"")+selector;}else if(name){ret.selector=this.selector+"."+name+"("+selector+")";}
return ret;};$.fn.pushStack=function(elems,name,selector){var ret;try{ret=pushStackOrig.call(this,elems,name,selector);return ret;}
catch(e){if(e instanceof TypeError){if(!(ret instanceof jQuery.fn.init)){ret=pushStackChrome.call(this,elems,name,selector);return ret;}}
throw e;}};}).call(this,jQuery);jQuery.fn.exists=function(){return(this.size()>0);};jQuery.fn.setClass=function(cssClass){var j=jQuery(this).attr('className',cssClass);return j;};jQuery.fn.isHidden=function(){if(jQuery(this).css('display')=='none')return true;return false;};jQuery.include=function(file,type){if(!type)type='script'
$.ajax({async:false,url:file,dataType:type,error:function(){var msg=new jqx.lib.Message('There was an error in loading the file'+file,'error');msg.display();}});};function initFuelNamespace(){var f;if(window.fuel==undefined){if(top.window.fuel!=undefined){f=top.window.fuel;}else{f={};}}else{f=window.fuel;}
return f;}
if(fuel==undefined){var fuel={};}
fuel.lang=function(key){return __FUEL_LOCALIZED__[key];}
...
$modal.jqm(jqmOpts).jqmShow();$modal.find('.modal_content').empty().append(html);$modal.find('iframe').load(function(){$('.jqmWindow .loader',$context).hide();var iframe=this;var contentDoc=iframe.contentDocument;$('.cancel',contentDoc).add('.modal_close').click(function(e){e.preventDefault();$modal.jqmHide();})
if(autoResize){setTimeout(function(){docHeight=fuel.calcHeight(contentDoc);$(iframe.contentWindow.parent.document).find('iframe').height(docHeight)
fuel.cascadeIframeWindowSize(docHeight);$(iframe).height(docHeight);},250);}})
return $modal;}
fuel.closeModal=function(){var modalId='__FUEL_modal__';$('#'+modalId).jqmHide();}
...
return level;}
fuel.calcHeight=function(context){var height=0;if($('#login',context).length){var elems='#login';}else{var elems='#fuel_main_top_panel, #fuel_actions, #fuel_notification, #fuel_main_content_inner, #list_container, .instructions';}
$(elems,context).each(function(i){height+=$(this).outerHeight();})
if(height>480){height=480;}else{height+=30;}
return height;}
fuel.adjustIframeWindowSize=function(){var iframe=$('.inline_iframe',top.window.document);if(iframe.length){iframe=iframe[0];var contentDoc=iframe.contentDocument;var height=fuel.calcHeight(contentDoc);var width=$('#fuel_main_content_inner .form',contentDoc).width()+50;$(iframe).height(height);$(iframe).width(width);}}
