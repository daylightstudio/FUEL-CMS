...
var $this=$(this).prev();var $clonecopy=$clone.clone(false);$clonecopy.addClass('noclone');$clonecopy.find(options.contentSelector+':first').show();if(dblclick=='accordian'){$prev.find(options.contentSelector).hide();}
...
var index=$children.length;parseTemplate($clonecopy,index);createRemoveButton($clonecopy);$this.append($clonecopy);$clonecopy.find('input,select,textarea').not('input[type="radio"], input[type="checkbox"], input[type="button"]').val('');$clonecopy.find('.noclone').remove();reOrder($this);if(max&&$children.length!=0&&$children.length>=(max-1)){$(this).hide();}
