...
$remove.find(options.removeButtonClass).length==0)$remove.empty().append('<a href="#" class="'+options.removeButtonClass+'">'+options.removeButtonText+" </a>");else{$remove.remove(options.removeSelector);$elem.append('<a href="#" class="'+options.removeButtonClass+'">'+options.removeButtonText+" </a>")}}$(document).on("click",options.repeatableSelector+" ."+options.removeButtonClass,function(e){var $this=$(this).closest(options.repeatableParentSelector);var max=$this.attr("data-max")?parseInt($this.attr("data-max")):
null;var min=$this.attr("data-min")?parseInt($this.attr("data-min")):null;if(options.warnBeforeDelete==false||confirm(options.warnBeforeDeleteMessage)){$(this).closest(options.repeatableSelector).remove();var $children=$this.children(options.repeatableSelector);if($children.length<max)$this.next().show();reOrder($this)}checkMin($this,min);$this.trigger("removed");e.stopImmediatePropagation();return false})};var reOrder=function($elem){$checked=$('input[type="radio"]',$elem).filter(":checked");$elem.children(options.repeatableSelector).each(function(i){$(this).attr("data-index",
