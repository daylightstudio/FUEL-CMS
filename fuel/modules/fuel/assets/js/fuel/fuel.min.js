...
var $select=$('#'+selectID),$optgroup=$select.find('optgroup');$select.find('option').each(function(i){var text=$(this).text(),value=$(this).attr('value'),id=(settings.optionsIdPrefix+idNum),opt='',$opt=$('<li/>');$opt.text(text);$opt.attr('id',id);if($(this).is(':disabled')){$opt.addClass('option_disabled');}
if(customSelectedSorting)
{var optSpanVal=(flippedSelectedOrder[value])?flippedSelectedOrder[value]:maxSelected;$opt.attr('data-label',text);$opt.append('<span/>');$opt.find('span').attr('id',id+'_val').text(optSpanVal).hide();}
