...
"$1_"+parentIndex+"_$2");$(this).attr("id",newId)}if(origName){var elemData=fuel.fields.getElementData(origName);if(elemData)fuel.fields.setElementData(newName,elemData)}})})};var createRemoveButton=function(elem){if(!options.removeable)return;$elem=$(elem);$elem.children(options.removeSelector).remove();$elem.append('<a href="#" class="'+options.removeButtonClass+'">'+options.removeButtonText+" </a>");$(document).on("click",options.repeatableSelector+" ."+options.removeButtonClass,function(e){var $this=
$(this).closest(options.repeatableParentSelector);var max=$this.attr("data-max")?parseInt($this.attr("data-max")):null;var min=$this.attr("data-min")?parseInt($this.attr("data-min")):null;if(options.warnBeforeDelete==false||confirm(options.warnBeforeDeleteMessage)){$(this).closest(options.repeatableSelector).remove();var $children=$this.children(options.repeatableSelector);if($children.length<max)$this.next().show();reOrder($this)}checkMin($this,min);$this.trigger("removed");e.stopImmediatePropagation();
return false})};var reOrder=function($elem){$checked=$('input[type="radio"]',$elem).filter(":checked");$elem.children(options.repeatableSelector).each(function(i){$(this).attr("data-index",i);parseTemplate(this,i)})};var checkMax=function($elem,max){if(max&&$elem.children(options.repeatableSelector).length!=0&&$elem.children(options.repeatableSelector).length>=max)$elem.next().hide();else $elem.next().show()};var checkMin=function($elem,min){$children=$elem.children(options.repeatableSelector);min=
parseInt(min);if(min&&$children.length!=0&&$children.length<=min)$children.find("."+options.removeButtonClass+":first").hide();else $children.find("."+options.removeButtonClass+":first").show()};var cloneRepeatableNode=function($elem){$clone=$elem.children(options.repeatableSelector+":first").clone(false);return $clone};var createCollapsingContent=function($elem){if(options.allowCollapsingContent)$($elem).find(options.sortableSelector).unbind("dblclick").dblclick(function(e){$parent=$(this).closest(options.repeatableSelector).parent();
var dblclick=$parent.attr("data-dblclick")?$parent.attr("data-dblclick"):null;var $elems=$(this).closest(options.repeatableSelector).find(options.contentSelector+":first");if(dblclick=="accordion"||dblclick=="accordian"){$parent.find(options.contentSelector).hide();$elems.show()}else $elems.toggle()})};var createAddButton=function($elem){$("."+options.addButtonClass).each(function(i){var $prev=$(this).prev();var $clone=cloneRepeatableNode($prev);$(this).data("clone",$clone)})};$(document).on("click",
"."+options.addButtonClass,function(e){e.preventDefault();e.stopImmediatePropagation();var $prev=$(this).prev();var max=$prev.attr("data-max")?parseInt($prev.attr("data-max")):null;var min=$prev.attr("data-min")?parseInt($prev.attr("data-min")):null;var dblclick=$prev.attr("data-dblclick")?$prev.attr("data-dblclick"):null;if(!$(e.currentTarget).data("clone")){var $clone=cloneRepeatableNode($prev);$(e.currentTarget).data("clone",$clone)}else var $clone=$(e.currentTarget).data("clone");var $this=$(this).prev();
var $clonecopy=$clone.clone(false);$clonecopy.addClass("noclone");$clonecopy.find(options.contentSelector+":first").show();if(dblclick=="accordian"||dblclick=="accordion")$prev.find(options.contentSelector).hide();createCollapsingContent($clonecopy);var $children=$this.children(options.repeatableSelector);if(max&&$children.length>=max)return false;var index=$children.length;parseTemplate($clonecopy,index);createRemoveButton($clonecopy);$this.append($clonecopy);$clonecopy.find("input,select,textarea").not('input[type="radio"], input[type="checkbox"], input[type="button"], .noclear').val("");
$clonecopy.find('input[type="checkbox"]').not(".noclear").prop("checked",false);$clonecopy.find('input[type="radio"]').not(".noclear").each(function(){if(parseInt($(this).data("orig_checked"))==1)$(this).prop("checked",true);else $(this).prop("checked",false)});$clonecopy.find(".noclone").remove();reOrder($this);if(max&&$children.length!=0&&$children.length>=max-1)$(this).hide();checkMin($prev,min);$this.trigger({type:"cloned",clonedNode:$clonecopy})});var index=0;return this.each(function(){var $this=
$(this);var $repeatables=$this.children(options.repeatableSelector);if(!$this.is(".__applied__"))$repeatables.each(function(i){parseTemplate(this,i);createRemoveButton(this)});$parent=$this.parent();if(options.max)$this.attr("data-max",options.max);if(options.min)$this.attr("data-min",options.min);if(options.dblClickBehavior)$this.attr("data-dblclick",options.dblClickBehavior);if(options.initDisplay&&!$this.is(".__applied__")){$this.attr("data-init_display",options.init_display);if($repeatables.closest(options.contentSelector).length)$toDisplay=
$repeatables.find(options.contentSelector);else $toDisplay=$repeatables.find(options.contentSelector).not(options.contentSelector+" "+options.contentSelector);if(options.initDisplay=="first")$toDisplay.not(":first").hide();else if(options.initDisplay=="none"||options.initDisplay=="closed")$toDisplay.hide()}if($parent.find(options.addButtonClass).length==0&&!$this.hasClass("__applied__"))$parent.append('<a href="#" class="'+options.addButtonClass+'">'+options.addButtonText+" </a>");if(options.sortable)$this.sortable({cursor:"move",
