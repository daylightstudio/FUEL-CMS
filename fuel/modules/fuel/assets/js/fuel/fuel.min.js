...
centerWidth+"px");var jqmOpts={onHide:modalOnHide,toTop:true};if(onLoadCallback)jqmOpts.onLoad=onLoadCallback;$modal.jqm(jqmOpts).jqmShow();$modal.find(".modal_content").empty().append(html);$modal.find("iframe").load(function(){$(".jqmWindow .loader",$context).hide();var iframe=this;var contentDoc=iframe.contentDocument;if(autoResize)setTimeout(function(){docHeight=fuel.calcHeight(contentDoc);if(iframe.contentWindow){$(iframe.contentWindow.parent.document).find("#"+modalId+"iframe").height(docHeight);
fuel.cascadeIframeWindowSize(docHeight);$(iframe).height(docHeight)}},250)});return $modal};fuel.closeModal=function(){var modalId="__FUEL_modal__";$("#"+modalId).jqmHide()};fuel.getModule=function(context){if(window.fuel&&window.fuel.module)return window.fuel.module;if(context==undefined)context=null;var module=$(".__fuel_module__",context).length?$(".__fuel_module__",context).val():null;return module};
fuel.getModuleURI=function(context){if(context==undefined)context=null;var module=$(".__fuel_module_uri__").length?$(".__fuel_module_uri__").val():null;return module};fuel.isTop=function(){return self==top};fuel.windowLevel=function(){var level=0;var win=window;while(win!=top&&win.parent!=null){level++;win=win.parent}return level};
