...
settings.wrapperId+" .supercomboselect_right_empty_msg").hide();if(!$("#"+leftID).children().length)$("#"+settings.wrapperId+" .supercomboselect_left_empty_msg").show();if(!$("#"+rightID).children().length)$("#"+settings.wrapperId+" .supercomboselect_right_empty_msg").show();isShiftDown=false;isCtrlDown=false;$(".supercomboselect").css({overflowY:"hidden"}).css({overflowY:"auto"}).disableTextSelect();autoSort()}function addSelectedToRight(){$selectedOpts=$("#"+leftID+" li[class="+settings.selectedClass+
"]");var selected=[];$selectedOpts.each(function(i){if(customSelectedSorting)theForm.find("#"+rightID).append($(this).removeClass(settings.selectedClass).html($(this).html()));var opt=$(getOptionSourceRef(getIdNum(this)));opt.attr("selected","selected");selected.push(opt.attr("value"))});$(_this).trigger("selectionAdded",[selected]);if(searchBox&&searchBox.val().length){searchBox.val("");searchBox.focus()}refreshLists()}function removeSelectedFromRight(){$selectedOpts=$("#"+rightID+" li[class="+settings.selectedClass+
"]");var selected=[];$selectedOpts.each(function(i){if(customSelectedSorting)$(this).remove();var opt=$(getOptionSourceRef(getIdNum(this)));opt.removeAttr("selected","selected");selected.push(opt.attr("value"))});$(_this).trigger("selectionRemoved",[selected]);refreshLists()}function markForMove(selector){if(isShiftDown&&lastSelected){var lis=$(selector).parents("ul").find("li");var num1=lis.index($(selector));var num2=lis.index($(lastSelected));if(num1<num2){startNum=num1;endNum=num2}else{startNum=
num2;endNum=num1}if(startNum>-1)lis.slice(startNum,endNum+1).addClass(settings.selectedClass)}else{$(selector).addClass(settings.selectedClass);lastSelected=selector}if(searchBox)searchBox.blur()}function getOptionSourceRef(idNum){return refs[idNum]}function getIdNum(selector){var id=$(selector).attr("id");var idNum=id?parseInt(id.substr(settings.optionsIdPrefix.length),10):0;return idNum}function autoSort(){if(settings.autoSort&&!settings.isSortable){var beginIndex=settings.optionsIdPrefix.length;
$("#"+rightID).find("li").selso({type:"alpha",extract:function(o){return $(o).text().toLowerCase()}})}}function onFormSubmit(){$("#"+rightID+" li").each(function(i){var src=$(getOptionSourceRef(getIdNum(this)));$(this).prepend('<input type="hidden" name="'+$("#"+selectID).attr("name")+'" value="'+src.attr("value")+'"" class="sorted_val" />')});$("#"+selectID).remove()}refreshLists(true);if(customSelectedSorting)$("#"+rightID).find("li").selso({type:"num",orderBy:"span",direction:"asc"})});return this}})(jQuery);
