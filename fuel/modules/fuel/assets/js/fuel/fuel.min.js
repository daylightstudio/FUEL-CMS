...
(function(a,b){if(a.cleanData){var c=a.cleanData;a.cleanData=function(b){for(var d=0,e;(e=b[d])!=null;d++)try{a(e).triggerHandler("remove")}catch(f){}c(b)}}else{var d=a.fn.remove;a.fn.remove=function(b,c){return this.each(function(){c||(!b||a.filter(b,[this]).length)&&a("*",this).add([this]).each(function(){try{a(this).triggerHandler("remove")}catch(b$0){}});return d.call(a(this),b,c)})}}a.widget=function(b,c,d){var e=b.split(".")[0],f;b=b.split(".")[1],f=e+"-"+b,d||(d=c,c=a.Widget),a.expr[":"][f]=
...
g){c.element.find(c._sanitizeSelector(e.hash)).html(f),c._cleanup(),d.cache&&a.data(e,"cache.tabs",!0),c._trigger("load",null,c._ui(c.anchors[b],c.panels[b]));try{d.ajaxOptions.success(f,g)}catch(h){}},error:function(a,f,g){c._cleanup(),c._trigger("load",null,c._ui(c.anchors[b],c.panels[b]));try{d.ajaxOptions.error(a,f,b,e)}catch(g$1){}}})),c.element.dequeue("tabs");return this}},abort:function(){this.element.queue([]),this.panels.stop(!1,!0),this.element.queue("tabs",this.element.queue("tabs").splice(-2,
...
if(b.input.val()!=b.lastVal)try{var c=$.datepicker.parseDate($.datepicker._get(b,"dateFormat"),b.input?b.input.val():null,$.datepicker._getFormatConfig(b));c&&($.datepicker._setDateFromField(b),$.datepicker._updateAlternate(b),$.datepicker._updateDatepicker(b))}catch(a$2){$.datepicker.log(a$2)}return!0},_showDatepicker:function(a){a=a.target||a,a.nodeName.toLowerCase()!="input"&&(a=$("input",a.parentNode)[0]);if(!$.datepicker._isDisabledDatepicker(a)&&$.datepicker._lastInput!=a){var b=$.datepicker._getInst(a);
...
doc.getElementsByTagName("pre")[0];var b=doc.getElementsByTagName("body")[0];if(pre)xhr.responseText=pre.textContent?pre.textContent:pre.innerText;else if(b)xhr.responseText=b.textContent?b.textContent:b.innerText}}else if(dt=="xml"&&!xhr.responseXML&&xhr.responseText!=null)xhr.responseXML=toXml(xhr.responseText);try{data=httpData(xhr,dt,s)}catch(e$4){status="parsererror";xhr.error=errMsg=e$4||status}}catch(e$5){log("error caught: ",e$5);status="error";xhr.error=errMsg=e$5||status}if(xhr.aborted){log("upload aborted");
...
y);if(currentRow)if(movingDown&&jQuery.tableDnD.dragObject!=currentRow)try{jQuery.tableDnD.dragObject.parentNode.insertBefore(jQuery.tableDnD.dragObject,currentRow.nextSibling)}catch(e){}else if(!movingDown&&jQuery.tableDnD.dragObject!=currentRow)try{jQuery.tableDnD.dragObject.parentNode.insertBefore(jQuery.tableDnD.dragObject,currentRow)}catch(e$6){}}return false},findDropTargetRow:function(draggedRow,y){var rows=jQuery.tableDnD.currentTable.rows;for(var i=0;i<rows.length;i++){var row=rows[i];var rowY=
...
$clonecopy.find("input,select,textarea").not('input[type="radio"], input[type="checkbox"], input[type="button"], .noclear').val("");$clonecopy.find('input[type="checkbox"]').not(".noclear").prop("checked",false);$clonecopy.find('input[type="radio"]').not(".noclear").each(function(){if(parseInt($(this).data("orig_checked"))==1)$(this).prop("checked",true);else $(this).prop("checked",false)});$clonecopy.find(".noclone").remove();reOrder($this);if(max&&$children.length!=0&&$children.length>=max-1)$(this).hide();
...
$context.append(modalHTML);$modal=$("#"+modalId,$context);$modal.attr("class","__fuel__ __fuel_modal__ jqmWindow "+cssClass);$(document).off("mousedown",".modal_close");$(document).on("mousedown",".modal_close",function(e){e.preventDefault();$(".modal_content").empty();$modal.jqmHide()});var modalOnHide=function(){$("#"+modalId,$context).hide();$(".jqmOverlay",$context).remove();if(onCloseCallback)onCloseCallback()};var modalWidth=$modal.outerWidth();var centerWidth=-(modalWidth/2);$modal.css("marginLeft",
centerWidth+"px");var jqmOpts={onHide:modalOnHide,toTop:true};if(onLoadCallback)jqmOpts.onLoad=onLoadCallback;$modal.jqm(jqmOpts).jqmShow();$modal.find(".modal_content").empty().append(html);$modal.find("iframe").load(function(){$(".jqmWindow .loader",$context).hide();var iframe=this;var contentDoc=iframe.contentDocument;$(".cancel",contentDoc).add(".modal_close").click(function(e){e.preventDefault();$modal.jqmHide()});if(autoResize)setTimeout(function(){docHeight=fuel.calcHeight(contentDoc);if(iframe.contentWindow){$(iframe.contentWindow.parent.document).find("#"+
modalId+"iframe").height(docHeight);fuel.cascadeIframeWindowSize(docHeight);$(iframe).height(docHeight)}},250)});return $modal};fuel.closeModal=function(){var modalId="__FUEL_modal__";$modal=$("#"+modalId,document.top);if($modal.find(".modal_close").length)$modal.find(".modal_close").trigger("mousedown");else $("#"+modalId).jqmHide()};
fuel.getModule=function(context){if(window.fuel&&window.fuel.module)return window.fuel.module;if(context==undefined)context=null;var module=$(".__fuel_module__",context).length?$(".__fuel_module__",context).val():null;return module};fuel.getModuleURI=function(context){if(context==undefined)context=null;var module=$(".__fuel_module_uri__").length?$(".__fuel_module_uri__").val():null;return module};fuel.isTop=function(){return self==top};
fuel.windowLevel=function(){var level=0;var win=window;while(win!=top&&win.parent!=null){level++;win=win.parent}return level};
fuel.calcHeight=function(context){var height=0;if($("#login",context).length)var elems="#login";else var elems="#fuel_main_top_panel, #fuel_actions, #fuel_notification, #fuel_main_content_inner, #list_container, .instructions, .modal_height";$(elems,context).each(function(i){var outerHeight=parseInt($(this).outerHeight(false));if(outerHeight)height+=outerHeight});if(height>600)height=600;else height+=30;return height};
