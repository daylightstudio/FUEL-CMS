...
min);$this.trigger({type:"cloned",clonedNode:$clonecopy})});var index=0;return this.each(function(){var $this=$(this);var $repeatables=$this.children(options.repeatableSelector);$repeatables.each(function(i){parseTemplate(this,i);createRemoveButton(this)});$parent=$this.parent();if(options.max)$this.attr("data-max",options.max);if(options.min)$this.attr("data-min",options.min);if(options.dblClickBehavior)$this.attr("data-dblclick",options.dblClickBehavior);if(options.initDisplay&&!$this.is(".__applied__")){$this.attr("data-init_display",
options.init_display);if(options.initDisplay=="first")$repeatables.find(options.contentSelector).not(":first").hide();else if(options.initDisplay=="none"||options.initDisplay=="closed")$repeatables.find(options.contentSelector).hide()}if($parent.find(options.addButtonClass).length==0&&!$this.hasClass("__applied__"))$parent.append('<a href="#" class="'+options.addButtonClass+'">'+options.addButtonText+" </a>");if(options.sortable)$this.sortable({cursor:"move",handle:options.sortableSelector,start:function(event,
ui){$this.trigger({type:"sortStarted",clonedNode:$this})},stop:function(event,ui){reOrder($(this));$this.trigger({type:"sortStopped",clonedNode:$this})}});if(!$this.hasClass("__applied__"))createAddButton($this);if($this.attr("data-max"))checkMax($this,options.max);if($this.attr("data-min"))checkMin($this,options.min);createCollapsingContent($this);index++;$this.addClass("__applied__");return this})}})(jQuery);
...
fuel.adjustIframeWindowSize=function(){var iframe=$(".inline_iframe",top.window.document);if(iframe.length){iframe=iframe[0];var contentDoc=iframe.contentDocument;var height=parseInt(fuel.calcHeight(contentDoc));var width=parseInt($("#fuel_main_content_inner .form",contentDoc).width())+50;$(iframe).height(height);$(iframe).width(width)}};fuel.cascadeIframeWindowSize=function(height){var level=0;if(height)height=