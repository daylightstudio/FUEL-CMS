	...
	<ul>
		<?php if (isset($action) AND $action == 'edit') : ?>
			...
			<?php if (!empty($this->preview_path) AND $this->fuel->auth->module_has_action('view')) : ?>
				<li><a href="<?=site_url($this->preview_path)?>" class="ico ico_view<?php if (!$this->fuel->config('view_in_new_window')) : ?> view_action<?php endif; ?>" title="<?=$keyboard_shortcuts['view']?> to view" target="view"><?=lang('btn_view')?></a></li>
			<?php endif; ?>
