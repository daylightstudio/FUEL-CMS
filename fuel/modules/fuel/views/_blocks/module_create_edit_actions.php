<div class="buttonbar" id="actions">
	<ul>
		<?php if (isset($action) && $action == 'edit') : ?>
			...
			<?php if (!empty($this->preview_path) && $this->fuel_auth->module_has_action('view')) : ?>
				<li><a href="<?=site_url($this->preview_path)?>" class="ico ico_view view_action" title="<?=$keyboard_shortcuts['view']?> to view">View</a></li>
			<?php endif; ?>

			<?php if ($this->fuel_auth->module_has_action('publish') && $this->fuel_auth->has_permission($this->permission, 'publish')) : ?>
				<?php if (!empty($publish)) : ?>
			<li><a href="#" class="ico ico_<?=strtolower($publish)?> <?=strtolower($publish)?>_action"><?=$publish?></a></li>
				<?php endif; ?>
			<?php endif; ?>
			<?php if ($this->fuel_auth->module_has_action('activate') && $this->fuel_auth->has_permission($this->permission, 'activate')) :  ?>
