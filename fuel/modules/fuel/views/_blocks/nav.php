...
<div id="fuel_left_panel">
			...
			foreach ($nav_items as $key => $val)
			{
				...
				if (($this->fuel->auth->has_permission($perm)) || $perm == 'dashboard')
				{
					if  ( ! $header_written)
					{
						...
						echo
						"<div class=\"left_nav_section\" id=\"leftnav_".str_replace('/', '_', $section)."\">\n".
						"\t<h3>".$section_hdr."</h3>\n".
						"\t<ul>\n";
					}
					...
					if (preg_match('#^'.$nav_selected.'$#', $url)) echo ' class="active"';
	...
	if ( ! empty($user_data['recent']))
	{
		...
		foreach ($user_data['recent'] as $val)
		{
			echo '<li><a href="'.site_url($val['l']).'" class="ico ico_'.$val['t'].'" title="'.$val['n'].'">'.$val['n'].'</a></li>';
		}
