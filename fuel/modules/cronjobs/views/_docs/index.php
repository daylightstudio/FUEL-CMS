...
<ol>
	<li>Make the file <dfn><?=INSTALL_ROOT?>crons/ci_cron.php</dfn> executable. This file will run the CodeIgniter bootstrap index.php file.</li>
	<li>Open up the <dfn><?=INSTALL_ROOT?>crons/ci_cron.php</dfn> and change the CRON_CI_INDEX. Optional constants to change are CRON_FLUSH_BUFFERS, CRON_TIME_LIMIT, $_SERVER['SERVER_NAME'], and the $_SERVER['SERVER_PORT'].</li>
...
This method only requires you to make the file <dfn><?=INSTALL_ROOT?>crons/crontab.php</dfn> writable.</p>
...
php <?=FCPATH?>index.php backup/cron

// using the ci_cron.php script that comes with FUEL
php <?=INSTALL_ROOT?>data_backup/ci_cron.php backup/cron
...
php <?=INSTALL_ROOT?>data_backup/ci_cron.php backup/cron/1
